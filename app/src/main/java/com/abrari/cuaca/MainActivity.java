...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        _recyclerView1 = (RecyclerView) findViewById(R.id.recylcerView1);
    ...
    private void initRecyclerView1(){
        String url = "http://api.openweathermap.org/data/2.5/forecast?id=1640118&appid=06b2264f9617e8b72b3608508b32bb75";
        AsyncHttpClient ahc = new AsyncHttpClient();

        ahc.get(url, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                Log.d("*al*", new String(responseBody));
                Gson gson = new Gson();
                RootModel rm = gson.fromJson(new String(responseBody), RootModel.class);
                Log.d("*al*", rm.getListModelList().get(0).getDt_txt());
